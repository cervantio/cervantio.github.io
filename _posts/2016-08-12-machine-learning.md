...
date: 2016-08-12
description: Using Machine Learning to detect Facial and Objects in a camera feed
categories:
  - IOT
  - Automation
image: https://www.awesome-automations.com/images/header_unsplash_32.jpg
...
echo Processing pictures...
set localdir=C:\user\facebox\person1
for /F %%x in ('dir /B/D %localdir%') do (
  set FILENAME=%localdir%\%%x
  echo Processing !FILENAME!
  curl -X POST -F file=@!FILENAME! "http://192.168.1.xxx:8080/facebox/teach?name=person1&id=person1"
)

echo Processing pictures...
set localdir=C:\User\facebox\person2
for /F %%x in ('dir /B/D %localdir%') do (
  set FILENAME=%localdir%\%%x
  echo Processing !FILENAME!
  curl -X POST -F file=@!FILENAME! "http://192.168.1.xxx:8080/facebox/teach?name=person2&id=person2"
)
